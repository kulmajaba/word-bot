strings = {
  "en": {
    "start": "Welcome to Visitorbot! Firstly, pick your language.",
    "usestart": "Start using the bot with the command /start.",
    "notregistered": "You have not registered to this bot yet. Complete the registration or use the command /start.",
    "usebuttons": "Please use the buttons or valid commands.",
    "changelang": "You can change the language to Finnish with the command /setlang fi and to Eglish with the command /setlang en.",
    "name": "Please write your first and last name.",
    "sthwrong": "Something went wrong.",
    "email": "Please write your email address.",
    "setupdone": "All done! You can use the Visitorbot with the command /buttons",
    "buttons": "Use the buttons to enter your visits.",
    "noauth": "You do not have the required access right.",
    "enter": "Enter",
    "exit": "Exit",
    "latestenter": "Current visit: entered",
    "latestexit": "Latest visit: exited",
    "help": "Show buttons: /buttons\n" \
            "Enter: /enter or use the button\n" \
            "Exit: /exit or use the button\n" \
            "Show user info: /getinfo\n" \
            "Edit your name: /setname Teemu Teekkari\n" \
            "Edit your email address: /setemail teemu.teekkari@esimerkki.fi\n" \
            "Change the language (options fi or en): /setlang en\n" \
            "Bot code: @kulmajaba",
    "adminmode": "\n\nYou are also designated as the admin of this bot. To get a list of all visits, use command /admin_get_report",
    "infoupdated": "Info updated succesfully."
  },

  "fi": {
    "start": "Tervetuloa Visitorbottiin! Valitse ensin kielesi.",
    "usestart": "Aloita käyttö komennolla /start.",
    "notregistered": "Et ole vielä rekisteröitynyt tähän bottiin. Tee rekisteröinti loppuun tai käytä komentoa /start.",
    "usebuttons": "Ole hyvä ja käytä nappeja tai komentoja.",
    "changelang": "Voit vaihtaa kielen englanniksi komennolla /setlang en ja suomeksi komennolla /setlang fi.",
    "name": "Ole hyvä ja kirjoita etu- ja sukunimesi.",
    "sthwrong": "Jotain meni pieleen.",
    "email": "Ole hyvä ja kirjoita sähköpostiosoitteesi.",
    "setupdone": "Valmista! Voit käyttää Visitorbottia komennolla /napit",
    "buttons": "Käytä nappeja kirjataksesi käyntisi.",
    "noauth": "Sinulla ei ole tarvittavaa käyttöoikeutta.",
    "enter": "Sisään",
    "exit": "Ulos",
    "latestenter": "Nykyinen käynti: saavuit",
    "latestexit": "Viimeisin käynti: poistuit",
    "help": "Näytä napit: /napit\n" \
            "Leimaa itsesi sisään: /sisaan tai käytä nappia\n" \
            "Leimaa itsesi ulos: /ulos tai käytä nappia\n" \
            "Näytä käyttäjätiedot: /getinfo\n" \
            "Muokkaa nimeäsi: /setname Teemu Teekkari\n" \
            "Muokkaa sähköpostiosoitettasi: /setemail teemu.teekkari@esimerkki.fi\n" \
            "Vaihda kieltä (vaihtoehdot fi tai en): /setlang en\n" \
            "Botin koodit: @kulmajaba",
    "adminmode": "\n\nSinut on myös merkattu tämän botin adminiksi. Hae lista kaikista käynneistä komennolla /admin_get_report",
    "infoupdated": "Tiedot päivitetty onnistuneesti."
  }
}